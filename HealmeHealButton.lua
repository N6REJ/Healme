function Healme_HealButton_OnLoad(frame)
...
function Healme_HealButton_OnUpdate(frame, elapsed)
	if ( not Healme.DoRangeChecks ) then return 0 end
	frame.TimeSinceLastUpdate = frame.TimeSinceLastUpdate + elapsed 	

	if (frame.TimeSinceLastUpdate > Healme.RangeCheckPeriod) then
		Healme_RangeCheckButton(frame)
		frame.TimeSinceLastUpdate = 0
	end
end

function Healme_HealButton_OnEnter(frame, motion)
	if (not Healme.ShowToolTips) then return end
    ...
    if frame.id and (stype == "spell") then 
		...
		if rank then 
			GameTooltip:AddLine(Healme_AddonColor .. rank .. "|r",1,1,1)
		end
	...
	elseif (stype == "macro") then 
		...
		GameTooltip:SetText("|cFFFFFFFFNo Spell|n|cFF00FF00You may drag-and-drop a spell from your|nspellbook onto this button, or you may go|nto Game Menu, Interface, Addons, " ..Healme_AddonName .. " and|nselect your spells from the list.")
    end
	
	GameTooltip:Show()			
end

function Healme_HealButton_OnLeave()
	GameTooltip:Hide()
end

function Healme_HealButton_OnEvent(frame, event)
...
local function PickupOldSpell(old) 
	...
	if IsShiftKeyDown() and (old.name ~= nil) then
		...
		if old.type == Healme_Type_Spell then
			PickupSpellBookItem(old.name)
			return
		end
		
		if old.type == Healme_Type_Macro then
			PickupMacro(old.name)
			return
		end
		
		if old.type == Healme_Type_Item then
			PickupItem(old.ID)
			return 
		end
	end
...
local function FinishDrag(frame,old)
	...
	Healme_UpdateButtonCooldownsByColumn(frame.index)
...
local function Drag(frame)
	if InCombatLockdown() then
		Healme_Warn("Can't update button while in combat")
		return
	end
	
	if (frame.index < 0) or (frame.index > Healme_MaxButtons) then
		return
	end

	local Profile = Healme_GetProfile()
	local infoType, info1, info2, info3 = GetCursorInfo()
	Healme_DebugPrint("infoType:", infoType, "info1:", info1, "info2:", info2, "info3:", info3 )
	...
	if infoType == "spell" then 
		...
		if info1 == 0 then  
			...
			Healme_DebugPrint("spellName:", spellName)
			if IsPassiveSpell(info1, BOOKTYPE_SPELL) then
				local link = GetSpellLink(info1, BOOKTYPE_SPELL)
				Healme_Warn(link .. " is a passive spell and cannot be used in " .. Healme_AddonName)
				return
			end
		end
		...
		local rankedSpellName = Healme_MakeRankedSpellName(spellName, subtext)
		Healme_DebugPrint("name:", spellName, "subtext:", subtext, "rankedSpellName:", rankedSpellName, "icon:", icon)
		Healme_SetProfileSpell(Profile, frame.index, spellName, info1, icon, subtext)
	...
	if infoType == "macro" then
		...
		Healme_SetProfileMacro(Profile, frame.index, name, info1, icon)
		FinishDrag(frame, old)
		return
	end
	...
	if infoType == "item" then
		...
		Healme_SetProfileItem(Profile, frame.index, name, info1, icon)
		FinishDrag(frame, old)
		return
	end
...
-- drag stop
function Healme_HealButton_OnReceiveDrag(frame)
	Healme_DebugPrint("Healme_HealButton_OnReceiveDrag() called")
	Drag(frame, nil)
end

-- drag start
function Healme_HealButton_OnDragStart(frame)
	Healme_DebugPrint("Healme_HealButton_OnDragStart() called")
