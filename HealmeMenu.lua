...
local function ShowMeFrame()
	Healme_ShowHideMeFrame(true)
end

local function ShowPetsFrame()
	Healme_ShowHidePetsFrame(true)
end

local function ShowDamagersFrame()
	Healme_ShowHideDamagersFrame(true)
end

local function ShowHealersFrame()
	Healme_ShowHideHealersFrame(true)
end

local function ShowTanksFrame()
	Healme_ShowHideTanksFrame(true)
end

local function ShowFriendsFrame()
	Healme_ShowHideFriendsFrame(true)
end

local function ShowTargetFrame()
	Healme_ShowHideTargetFrame(true)
end

local function ShowFocusFrame()
	Healme_ShowHideFocusFrame(true)
end

local function CanConfigureButtons()
	if InCombatLockdown() then
		Healme_Warn("Can't configure buttons while in combat!")
		return false
	end
...
local function SetCurrentSpell(info, btnIndex, spellIndex)
	...
	Healme_SetProfileSpell(Profile, btnIndex, Healme_Spell.Name[spellIndex], Healme_Spell.ID[spellIndex], Healme_Spell.Icon[spellIndex])
...
local function InsertButton(info, btnIndex)
	...
	local Profile = Healme_GetProfile()
	if Profile.ButtonCount >= Healme_MaxButtons then
		Healme_Warn("Can't insert more buttons! Max button count already reached!")
		return
	end
...
local function HealmeMenu_InitializeDropDown(frame,level)
	level = level or 1
	...
	local MenuTable = 
	{
		...
		{
			{ -- Title
				text = Healme_AddonColor .. Healme_AddonName .. "|r Menu",
				isTitle = 1,
				notCheckable = 1,
			},
			{ -- Config Panel
				text = Healme_AddonName .. " Config Panel",
				func = Healme_ShowConfigPanel,
				notCheckable = 1,
			},
			{ -- Set button count
			...
			{ -- Reset all frame positions
				...
				func = Healme_ResetAllFramePositions,
				notCheckable = 1,
			},
			{ -- Close
				hasArrow  = nil,
...
--]]				
				...
				},
				{
					...
					notCheckable = 1,					
					func = Healme_HideAllRaidFrames,
				},
				{
					text = "Show Raid Groups 1 and 2 (10 man)",
					notCheckable = 1,					
					func = Healme_Show10ManRaidFrames,
				},
				{
					text = "Show Raid Groups 1-5 (25 man)",
					notCheckable = 1,					
					func = Healme_Show25ManRaidFrames,
				}, 
				{
					text = "Show Raid Groups 1-8 (40 man)",
					notCheckable = 1,					
					func = Healme_Show40ManRaidFrames,
				}, 
			},
		},
		[3] = {},
	...
	for i=0, Healme_MaxButtons, 1 do
...
--			menuItem.disabled = nil
	...
	{
		{-- Title
			text = Healme_AddonColor .. Healme_AddonName .. "|r Button " .. index,
			isTitle = 1,
			notCheckable = 1
		}
	}
	...
	if not IsClassic then 
		for k, v in ipairs (Healme_Spell.Name) do
			local spellmenuItem = { }
			spellmenuItem.text = Healme_Spell.Name[k]
			spellmenuItem.func = SetCurrentSpell
			spellmenuItem.icon = Healme_Spell.Icon[k]
			spellmenuItem.checked = currentSpell == Healme_Spell.Name[k]
...
function Healme_InitMenu()
	HealmeMenu = CreateFrame("Frame", "HealmeOptionsMenu", Healme_MMButton, "Lib_UIDropDownMenuTemplate")
	HealmeMenu:SetPoint("TOP", Healme_MMButton, "BOTTOM")
	Lib_UIDropDownMenu_Initialize(HealmeMenu, HealmeMenu_InitializeDropDown, "MENU");
end